...
function loadEnJSON(data) {
  ...
  for (let p of data.ArchitecturalProject) {
    projectList.push(
...
function mainScreen() {
  ...
  for (let x = 0; x < projectList.length; x++) {
    $(".WK-scrollWrap").append(
      ...
      <a href="galleryA.html" class="WK-projectPageLink" id='${projectList[x].projectId}'>
          <img src="images/architecture/${projectList[x].titleImage}" alt="image of an architecture thing"
